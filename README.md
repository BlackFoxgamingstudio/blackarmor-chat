# Sovereign Blackarmor Chat (`sovereign-blackarmor-chat`)
...
[![CI](https://github.com/BlackFoxgamingstudio/blackarmor-chat/actions/workflows/ci.yml/badge.svg)](https://github.com/BlackFoxgamingstudio/blackarmor-chat/actions/workflows/ci.yml)
[![n8n Integration](https://img.shields.io/badge/n8n-workflow_ready-orange.svg)](n8n/workflow.json)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](LICENSE)

> **Enterprise Standalone Package**: Secure, self-hosted web collaboration and chat interface with CI/CD deployment automation. Features document collaboration integration, real-time message broadcasting, and encrypted database persistence.
...
| **REST Gateway** | ✅ Live on Port `8804` | `POST /api/v1/execute`, `GET /health` |
...
`sovereign-blackarmor-chat` is an independently packaged, zero-dependency software library and microservice engineered as part of Russell Alan Powers' 10-year software engineering portfolio.

It delivers robust capabilities in **Enterprise Web Chat & Real-Time Sync** and provides seamless integration with n8n event workflows.

```
┌───────────────────────────┐         HTTP POST          ┌───────────────────────────────────────────┐
│        n8n Engine         │ ─────────────────────────> │        sovereign-blackarmor-chat Adapter        │
│   (Port 5678 Webhook)     │ <───────────────────────── │             (Port 8804)                 │
└───────────────────────────┘       Idempotent JSON      └───────────────────────────────────────────┘
                                                                               │
                                                                               ▼
                                                         ┌───────────────────────────────────────────┐
...
- **Primary Module**: `from sovereign_blackarmor_chat import ChatServer, DocumentCollaborator, BroadcastHub, MessageCryptor`
...
git clone https://github.com/russellpowers/sovereign-blackarmor-chat.git
cd blackarmor-chat
...
sovereign-blackarmor-chat --health
```
...
sovereign-blackarmor-chat --exec process_data --payload '{"sample_key": "sample_value"}'
```
...
- **Microservice Port**: `http://localhost:8804`
- **Inbound Trigger Route**: `POST /api/v1/execute`
- **Integration Workflow**: `Incoming customer ticket -> Stream chat message to Blackarmor interface -> Dispatches bot agent response via WebSocket`
...
4. Start the background webhook adapter:
   ...
   ```
5. Dispatch your test event to `http://localhost:5678/webhook/blackarmor-chat-trigger`.
