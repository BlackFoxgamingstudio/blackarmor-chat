# Developer Guide: Sovereign BlackArmor Chat
...
```bash
# 1. Clone
git clone https://github.com/BlackFoxgamingstudio/blackarmor-chat.git
cd blackarmor-chat
...
```bash
export SBB_CHAT_SECRET_KEY=your_value_here
export SBB_REDIS_URL=your_value_here
export SBB_MAX_CONNECTIONS=your_value_here
export SBB_ENCRYPTION_ALGO=your_value_here
...
│   ├── cli.py               # Typer CLI (sovereign-blackarmor-chat)
...
```bash
# Build
docker build -t sovereign-blackarmor-chat:dev .

# Run with env file
docker run --env-file .env -p 8804:8804 sovereign-blackarmor-chat:dev

# Full stack (with n8n network)
docker-compose up --build

# Logs
docker-compose logs -f sovereign-blackarmor-chat
...
3. Use **SovereignTools** node → configure endpoint to `http://localhost:8804`

### Triggering Directly

```bash
curl -X POST http://localhost:8804/api/v1/execute \
...
requests.post(N8N_EVENT_BUS_URL, json={
    "source": "sovereign-blackarmor-chat",
...
| Issue | Diagnosis | Fix |
|-------|-----------|-----|
| Port `8804` in use | `lsof -i :8804` | Kill PID or change `SERVICE_PORT` |
